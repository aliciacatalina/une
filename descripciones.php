        ...
        if(loggedin()==true)
		{
            ...
            if($log->fetch()) //fetching the contents of the row
            {
                if($user_level=='superadmin')
				{
					...
					$res = $con->query("SELECT descripcion, idDescripcion FROM descripcion WHERE idTitulo=".$_GET['title']." AND idCampus =".$_GET['idCampus']);
					$rows = array();
					$trows = array();
					...
					foreach($rows as $row)
					{
						...
						echo '<div name="descripcion"> '.$descripcion.' </div>';
					}
                }
